...
# Constants
TARGET_CHANNEL_ID = 1239229516300419074  # ReadyCheck Channel ID
...
class RoleButton(discord.ui.Button):
    def __init__(self):
        super().__init__(label=f"Ready", style=discord.ButtonStyle.primary, custom_id="ready_button")
...
class RoleButtonMenu(commands.Cog):
    ...
    async def on_ready(self):
        ...
        pprint('ReadyCheck cog loaded and view registered')
    ...
    async def menu(self, interaction: discord.Interaction):
        ...
        if target_channel is None:
            await interaction.followup.send(f"Target channel with ID {TARGET_CHANNEL_ID} not found.", ephemeral=True)
            return
        ...
        await interaction.followup.send(f"Message sent to {target_channel.mention}, and {HARDCODED_ROLE_NAME} removed from all users.", ephemeral=True)
